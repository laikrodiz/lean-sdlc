...
Assisted mode is the default. The user-selected lead keeps architecture, interfaces, task setting, integration, and closeout. Codex gives one durable task beyond the direct fast path to one reusable Executor. Executor performs local implementation and fast targeted checks. Executor returns one task checkpoint. The lead reviews architecture, scope, and diff once per returned checkpoint. Corrections return as concise deltas to the same Executor.
...
```bash
git clone --depth 1 --branch v1.5.0 https://github.com/laikrodiz/lean-sdlc.git
cd lean-sdlc
