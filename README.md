...
Every primary child uses the named `lean_sdlc_luna` profile. Primary Luna spawns use `agent_type=lean_sdlc_luna`, `service_tier=fast`, and non-full-history context. If fast is unavailable or rejected, the lead announces the failure and retries Luna Max without `service_tier`. Terra `xhigh` is the direct fallback without `service_tier`. Sol and Terra never use fast unless the user explicitly overrides it. The lead preserves the user-selected lead model and tier.
