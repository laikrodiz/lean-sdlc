...
| Researcher | Evidence spans multiple sources, repositories, large documents, data, logs, or noisy output. | Read-only, source-bounded research with cited findings and conflicts. |
...
- Before replacement announce `Role`, `Identity`, `Context reset reason`, and `Replacement action`. Keep the role prefix and choose another unused first name.
...
- The Architect supplies a question and source boundary without a preferred answer. Researcher is read-only and never edits repository files.
- A Researcher handoff uses `Role: Researcher`, `Identity: Firstname (Researcher)`, `Inquiry:`, `Task name:`, and `Decision informed:`.
- Researcher returns Cited findings, Conflicts, Unknowns, Decision impact, and Sources. The Architect evaluates the evidence and retains the decision.
...
- A Researcher inquiry needs no task until its findings require a repository write.
...
- The profile receives the Engineer, Maintainer, Verifier, or Researcher role. Fast service maps to `service_tier=priority`.
...
3. Choose an unused simple human first name. Show role, authority, identity, and task name.
4. Apply the one-thread lifecycle, depth-one limit, named profile, Fast-service rule, handoff, and reporting contract.
5. Reject vague, duplicate, feature-based, counter-based, or task-identifier names.
...
- Engineer edits only assigned paths under the active Architect-owned task. Sidecars do not edit source or `tasks.csv`; Researcher never edits repository files.
...
- Researcher adds `Cited findings:`, `Conflicts:`, `Unknowns:`, `Decision impact:`, and `Sources:`. Maintainer adds operation commands and artifacts.
