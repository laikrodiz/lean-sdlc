...
- Verifier: run tests, lint, type checks, structural checks, and diff inspection. Return evidence or specific failures. Never repair source or close tasks.
- Operator: run build, package, CI, deploy, flash, runtime, and smoke procedures. Compress logs and identify artifacts and targets. Never repair source, invent procedures, or close tasks.
...
Reference files and saved logs instead of pasting large content. Preserve exact decisions, commands, fingerprints, failures, and evidence; omit exploration chatter.
