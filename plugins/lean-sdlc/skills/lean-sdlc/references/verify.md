...
1. Read the active task and source boundary completely, with acceptance and proof. Require exact proof or a recorded procedure.
...
8. Treat task proof as the acceptance anchor. Retain required acceptance, regression, structure, and documentation layers.
9. Stop after all required proof passes. Add an always-on rule only for an observed failure. Use the smallest behavioral evaluation that fails before the rule and passes after it.
