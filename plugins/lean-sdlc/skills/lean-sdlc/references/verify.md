...
Spawn the Verifier with explicit Luna `xhigh`, or explicit Terra `high` when Luna is unavailable, to execute and summarize checks. The Sol `medium` or `high` lead makes the final close, fail, reopen, or truth-resolution decision.
...
3. Have the Verifier run the promised checks against that exact checkpoint and report operation pass/fail plus evidence; never ask it for task pass/block or closure. Run checks locally in Solo mode.
