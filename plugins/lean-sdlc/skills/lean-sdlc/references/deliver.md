...
10. Before the Executor handoff, show the concise architecture brief required by [subagents.md](subagents.md). Apply the Executor trigger only after the visible plan exists and the task matches it. In Assisted mode, delegate one ready durable task beyond the direct fast path to the existing `executor_david` thread; otherwise spawn it lazily or execute locally.
...
14. After Executor returns one task checkpoint, inspect its diff and contract alignment, then send the concise lead sign-off required by [subagents.md](subagents.md).
15. Accept the checkpoint only after sign-off, or send a concise correction delta to the same Executor before defining another task.
