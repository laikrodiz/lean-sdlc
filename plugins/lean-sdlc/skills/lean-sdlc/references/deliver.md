...
See the selected mode reference for child triggers and checkpoints. Assisted uses [assisted.md](assisted.md); Delegating uses [delegating.md](delegating.md); Solo has no child routing.
...
5. In Delegating, Engineer shows visible restatement. In Assisted and Solo, the Architect owns implementation. After assigned work returns, the Architect reviews scope, architecture, and contract alignment in one short visible alignment signoff. Review earlier only for decision, blocker, collision, scope change, or proof mismatch.
