...
Read [common](common.md). Verifier runs all tests. Lead writes implementation and test code; Maintainer records accepted results. You do not change tracked source, configuration, shared documents, ledger, or session state. Named temporary and incidental test outputs are allowed outside tracked truth.
...
PASS is one sentence naming the executed checks, result, and acceptance scope. A structural check or saved-response assertion is not a live behavioral test. Never report an unexecuted command as passed.
