...
Confirm required task creation, ownership, and write-gate results promptly. Prioritize ledger transactions over long documentation or operation work. Batch related safe transactions and reuse this agent. Do not create a queue service or new task database.

Lead may show a requested status before the transaction finishes. Report a transaction failure immediately with the task, actual state, and blocked next action. Never fabricate successful closure. Close only after Lead accepts the required proof. At final reconciliation, compare the requested task table with actual ledger rows and apply only missing authorized transactions.
...
Return the completed ledger action, documentation result, or operation result once. Report failures and decisions immediately. Do not run tests; Verifier owns test execution, including test steps inside delivery procedures.
