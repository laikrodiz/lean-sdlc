...
- The contract upgrader recognizes the exact released `v1.26.0` template. It replaces only the managed prefix and preserves appended custom project rules. Edited or unknown core instructions require explicit manual reconciliation. Do not overwrite custom rules or interpret application changes as migration authority. After an authorized upgrade, reload current repository and selected workflow instructions in the same session. Preserve ownership, custom rules, current mode, and valid evidence. Resume only after conflict, before-write, and required activation checks pass. A new session requires a specific unresolved instruction or runtime conflict, not merely an upgrade. Hooks validate covered spawn arguments only; they do not establish identity or prevent canceled child writes.
...
- Stage: Evolution
- Version: 1.27.2
- Version goal: Continue in the same task after authorized repository upgrades and instruction updates. Retain explicit same-session mode changes. Preserve risk-based proof, permissions, ownership, atomic acceptance, ledger, and bytecode boundaries.
